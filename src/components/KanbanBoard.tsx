...
export default function KanbanBoard() {
  ...
  if (!activeBoard) return <div className={`flex-1 flex items-center justify-center ${isDark ? "text-slate-500" : "text-slate-400"}`}>Crie ou selecione um board.</div>;
  ...
  return (
    <div className="flex-1 h-full overflow-x-auto p-6 flex gap-6">
      ...
      <div className="min-w-[300px] shrink-0">
        <div className={`border border-dashed rounded-xl p-3 flex gap-2 transition-colors ${isDark ? "bg-slate-800/50 border-slate-700" : "bg-slate-100/50 border-slate-300"}`}>
          <input value={newColTitle} onChange={e => setNewColTitle(e.target.value)} placeholder="Nova Coluna..."
            className={`flex-1 bg-transparent outline-none px-2 text-sm transition-colors ${isDark ? "text-slate-100 placeholder-slate-500" : "text-slate-900 placeholder-slate-400"}`}
            onKeyDown={e => { if(e.key === "Enter" && newColTitle) { createColumn(newColTitle); setNewColTitle(""); } }} />
          <button onClick={() => { if(newColTitle) { createColumn(newColTitle); setNewColTitle(""); } }} className={`p-1 transition-colors ${isDark ? "text-slate-400 hover:text-slate-200" : "text-slate-500 hover:text-slate-800"}`}>
            <Plus size={20} />
          </button>
        </div>
      </div>
...
function ColumnView({ column, onCardClick }: { column: Column, onCardClick: (card: Card) => void }) {
  ...
  return (
    <div className={`flex flex-col rounded-2xl w-[320px] shrink-0 max-h-full transition-colors ${isDark ? "bg-slate-800" : "bg-slate-100"}`}>
      <div className={`p-4 flex justify-between items-center group ${isDark ? "border-b border-slate-700" : ""}`}>
        <h3 className={`font-semibold ${isDark ? "text-slate-100" : "text-slate-900"}`}>{column.title} <span className={`text-xs font-normal ${isDark ? "text-slate-500" : "text-slate-400"} ml-2`}>{column.cards.length}</span></h3>
      ...
      <div ref={setDroppableNodeRef} className={`flex-1 overflow-y-auto px-3 pb-3 space-y-3 custom-scrollbar transition-colors ${isOver ? (isDark ? 'bg-slate-700' : 'bg-slate-200/70') : ''}`}>
        ...
        {isAdding ? (
          <div className={`p-3 rounded-xl shadow-sm border transition-colors ${isDark ? "bg-slate-700 border-slate-600" : "bg-white border-blue-200"}`}>
            <input autoFocus value={newTitle} onChange={e => setNewTitle(e.target.value)} placeholder="Título da tarefa..."
              className={`w-full text-sm outline-none mb-2 transition-colors ${isDark ? "bg-slate-600 text-slate-100 placeholder-slate-400" : "bg-white text-slate-900 placeholder-slate-400"}`}
              onKeyDown={e => { if(e.key === "Enter" && newTitle) { createCard(column.id, newTitle); setNewTitle(""); setIsAdding(false); } }} />
            <div className="flex gap-2">
              <button onClick={() => { if(newTitle) createCard(column.id, newTitle); setNewTitle(""); setIsAdding(false); }} className="text-xs bg-blue-600 text-white px-3 py-1.5 rounded-lg hover:bg-blue-700">Salvar</button>
              <button onClick={() => setIsAdding(false)} className={`text-xs px-3 py-1.5 rounded-lg transition-colors ${isDark ? "text-slate-300 hover:bg-slate-600" : "text-slate-500 hover:bg-slate-100"}`}>Cancelar</button>
            </div>
          </div>
        ) : (
          <button onClick={() => setIsAdding(true)} className={`w-full py-2.5 flex items-center justify-center gap-2 text-sm font-medium rounded-xl transition-colors ${isDark ? "text-slate-400 hover:bg-slate-700 hover:text-slate-300" : "text-slate-500 hover:bg-slate-200/50 hover:text-slate-900"}`}>
    ...
    <div ref={setNodeRef} style={style} {...attributes} className="relative group">
      ...
      <div onClick={onClick} className={`p-4 rounded-xl shadow-sm hover:shadow-md cursor-pointer pl-8 transition-shadow border-l-4 ${isDark ? "bg-slate-700 border border-slate-600 hover:border-slate-500" : "bg-white border border-slate-100"}`}
        ...
        {card.comments.length > 0 && (
          <div className={`flex items-center gap-1 mt-3 text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}>
