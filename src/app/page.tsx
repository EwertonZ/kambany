...
import { LayoutDashboard, Download, Upload, Plus, Trash2 } from "lucide-react";
    ...
    <main className="flex h-screen w-full">
      ...
      <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col h-full border-r border-slate-800 shrink-0">
        ...
        <div className="px-4 py-2">
          ...
          <div className="mt-4 px-2 flex gap-2">
            <input value={newBoardName} onChange={e => setNewBoardName(e.target.value)} placeholder="Novo board..."
              className="w-full bg-slate-800 text-sm text-white px-3 py-2 rounded-lg outline-none border border-slate-700 focus:border-blue-500"
        ...
        <div className="mt-auto p-6 space-y-3">
          <button onClick={exportData} className="w-full flex items-center justify-center gap-2 text-sm bg-slate-800 hover:bg-slate-700 py-2.5 rounded-xl transition-colors">
            <Download size={16} /> Exportar Backup
          </button>
          <button onClick={() => fileInputRef.current?.click()} className="w-full flex items-center justify-center gap-2 text-sm bg-slate-800 hover:bg-slate-700 py-2.5 rounded-xl transition-colors">
      ...
      <section className="flex-1 flex flex-col bg-slate-50 min-w-0">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center px-8 shadow-sm z-10 shrink-0">
          <h2 className="text-lg font-bold text-slate-800">{activeBoard?.name || "Nenhum Board Selecionado"}</h2>
        </header>
        <KanbanBoard />
      </section>
    </main>
